...
import { faPlus, faMinus, faTruck, faRotate } from '@fortawesome/free-solid-svg-icons'
  ...
  useEffect(() => {
    if(productDetails?.category?.length) {
      ...
      const relatedProducts = products.filter(product => {
        ...
        return productCategories.some(category => categories.includes(category));
      })
        ...
        {!isLoaded 
          ...
          : <section className='flex flex-col md:flex-row justify-between gap-20'>
              ...
              <div className="product-details flex flex-col gap-4 flex-grow basis-1/4">
                <h1 id='product-name' className='font-bold text-2xl'>{productDetails.name}</h1>

                <span id='ratings' className='flex text-sm gap-2 items-center flex-wrap'>
                  <img src={`../images/ratings/rating-${productDetails.ratings.stars * 10}.png`} alt="star" className='w-20 h-4'/>
                  <p className='text-secondaryGray'>({productDetails.ratings.count})</p>
                  <p>|</p>
                  <p className='text-buttonLime'>{productDetails.stock_status}</p>
                </span>

                <h2 id='price' className='text-2xl'>${productDetails.price}</h2>

                <p id="description" className="text-stat">{productDetails.description}</p>
                ...
                {productDetails.size && <span id="size" className='flex items-center gap-2'>
                  ...
                  {productDetails.size.map(size => <span key={size} className='block items-center p-1.5 border border-secondaryGray rounded-md text-sm'>{size.toUpperCase()}</span>)}
                ...
                <div className="flex flex-wrap items-stretch justify-between gap-4">
                  <div id="quantity" className='flex flex-grow basis-42'>
                    <button id="increment" className='w-11 h-11 text-lg border border-secondaryGray rounded-tl-md rounded-bl-md hover:bg-primaryRed hover:text-white duration-200' onClick={decrementQuantity}>
                      <FontAwesomeIcon icon={faMinus}/>
                    </button>
                    <input type="text" value={quantityValue} onChange={(e) => setQuantityValue(e.target.value)} className='bg-transparent border-y border-secondaryGray flex-grow w-10 text-center outline-none'/>
                    <button id="increment" className='w-11 h-11 text-lg border border-secondaryGray rounded-tr-md rounded-br-md hover:bg-primaryRed hover:text-white duration-200' onClick={incrementQuantity}>
                      <FontAwesomeIcon icon={faPlus}/>
                    </button>
                  </div>

                  <button id="buy-btn" className='py-2 bg-primaryRed text-white h-bg-red font-semibold flex-grow rounded-md'>
                    Buy Now
                  </button>

                  <button id="wishlist" className='w-11 h-11 text-2xl border border-secondaryGray rounded-md hover:bg-primaryRed hover:text-white duration-200' onClick={() => addToWishlist(productId)}>
                    <FontAwesomeIcon icon={faHeart} />
                  </button>
                </div>

                {/* SERVICES */}
                <div id="services" className="flex flex-col mt-5 rounded-md border border-black">
